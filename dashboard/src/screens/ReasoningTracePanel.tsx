...
export default function ReasoningTracePanel() {
  return (
    <div className="bg-background text-on-background font-body text-body min-h-screen overflow-hidden antialiased relative">
      <style>{`
      ...
      `}</style>
      <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1551288049-bebda4e38f71?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center opacity-20 filter blur-sm pointer-events-none" data-alt="A clean, modern clinical dashboard interface displaying patient data, vital signs charts, and medical history in a highly structured, dense grid layout. The colors are muted whites and greys with subtle teal accents, representing a high-end medical software system."></div>
      <div className="absolute inset-0 bg-inverse-surface/40 backdrop-blur-md z-40 transition-opacity"></div>
      <div className="fixed top-0 right-0 h-screen w-[480px] bg-surface-container-lowest border-l border-outline-variant/30 shadow-[-4px_0_24px_rgba(0,0,0,0.05)] z-50 flex flex-col transform transition-transform duration-300 translate-x-0">
        <div className="flex-none p-card_padding border-b border-outline-variant/30 bg-surface-container-lowest sticky top-0 z-10">
          <div className="flex items-start justify-between mb-4">
            ...
            <button className="p-1.5 rounded-md text-on-surface-variant hover:bg-surface-container-high transition-colors -mr-1.5 -mt-1.5 focus:outline-none">
              <span className="material-symbols-outlined text-[20px]">close</span>
            </button>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-card_padding space-y-container_margin bg-surface-bright">
          <FadeIn delay={0}>
          <div className="flex-none px-card_padding py-4 border-b border-outline-variant/30 bg-surface-container-low/50">
          ...
          <section className="space-y-4">
            ...
            <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-lg overflow-hidden">
              <ul className="divide-y divide-outline-variant/30">
                <li className="p-3 flex items-start gap-3 hover:bg-surface-container-low transition-colors group">
